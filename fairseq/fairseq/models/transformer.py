...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(
        ...
        if alignment_layer is None:
            alignment_layer = 0
            # alignment_layer = 3
            # alignment_layer = self.num_layers - 3
        ...
        if inputs_embed is not None:
            x = self.embed_scale * inputs_embed
        else:
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.decoder_layerdrop):
                if isinstance(mask_position, dict):
                    mask_position_i = mask_position[idx] if idx in mask_position else None
                else:
                    mask_position_i = mask_position if idx == alignment_layer else None
                if mask_position_i is not None:
